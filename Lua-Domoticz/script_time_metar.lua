...
print('script_time_metar.lua')
	 ...
	 local annee, mois, jour, h, m, s = string.match (obs_time, "(%d+)-(%d+)-(%d+)T(%d+):(%d+):(%d+)Z")
	 ...
	 local angle_degres = tonumber (obs_windir)
		 ...
		 if angle_degres == 0 then
			 direction = "Variable"
		 elseif angle_degres >= 1 and angle_degres < 11.5 then
			 direction = "N"
		 elseif angle_degres >= 11.5 and angle_degres < 34 then
			 direction = "NNE"
		 elseif angle_degres >= 34 and angle_degres < 56.5 then
			 direction = "NE"
		 elseif angle_degres >= 56.5 and angle_degres < 79 then
			 direction = "ENE"
		 elseif angle_degres >= 79 and angle_degres < 90 then
			 direction = "E"
		 elseif angle_degres >= 90 and angle_degres < 101.5 then
			 direction = "E"
		 elseif angle_degres >= 101.5 and angle_degres < 124 then
			 direction = "ESE"
		 elseif angle_degres >= 124 and angle_degres < 146.5 then
			 direction = "SE"
		 elseif angle_degres >= 146.5 and angle_degres < 169 then
			 direction = "SSE"
		 elseif angle_degres >= 169 and angle_degres < 180 then
			 direction = "S"
		 elseif angle_degres >= 180 and angle_degres < 191.5 then
			 direction = "S"
		 elseif angle_degres >= 191.5 and angle_degres < 214 then
			 direction = "SSO"
		 elseif angle_degres >= 214 and angle_degres < 236.5 then
			 direction = "SO"
		 elseif angle_degres >= 236.5 and angle_degres < 259 then
			 direction = "OSO"
		 elseif angle_degres >= 259 and angle_degres < 270 then
			 direction = "O"
		 elseif angle_degres >= 270 and angle_degres < 281.5 then
			 direction = "O"
		 elseif angle_degres >= 281.5 and angle_degres < 304 then
			 direction = "ONO"
		 elseif angle_degres >= 304 and angle_degres < 326.5 then
			 direction = "NO"
		 elseif angle_degres >= 326.5 and angle_degres < 349 then
			 direction = "NON"
		 elseif angle_degres >= 349 and angle_degres <= 360 then
			 direction = "N"
		 end
	 ...
	 if obs_wxstat ~= nil then
		 ...
		 for wx in string.gmatch(obs_wxstat, "%a+") do
			 ...
			 -- conversion valeurs --
			 local wx_var_1 = weather_type(table_1)
			 print(wx_var_1)
